...
public class AD_FileSystemItemInfo : AD_SerializableItemInfo
    , IAD_FileSystemItemInfo
{
    ...
    public AD_FileSystemItemInfo(AD_FileSystemItemInfo otherInst)
    {
        CopyBaseMembersFromFunc(otherInst);
    }
    public AD_FileSystemItemInfo(string name, AD_ShellType itemShellType, string selfPath, AD_SpeicalFolder specialFolder = AD_SpeicalFolder.None, AD_ShellType targetShellType = AD_ShellType.None, string targetPath = "", Texture2D texturePreview = null)
    {
        this.name = name;
    ...
    {
        if (otherInst is AD_FileSystemItemInfo otherItemInfo)
            CopyBaseMembersFromFunc(otherItemInfo);
    }
    void CopyBaseMembersFromFunc(AD_FileSystemItemInfo otherInst)
    {
        name = otherInst.name;
    ...
    public override bool Equals(object other)
    {
        ...
        if ((other == null)/* || !GetType().Equals(other.GetType())*/)//-只判断关键字段，不直接判断引用类型（因为可能是不同实例）
            return false;
        else if (!(other is AD_FileSystemItemInfo))//非本类型
            return false;
        else
            return EqualsFunc((AD_FileSystemItemInfo)other);
    }
    /// <summary>
    /// 值是否相同
    /// 
    ...
    /// <param name="other"></param>
    /// <returns></returns>
    protected virtual bool EqualsFunc(AD_FileSystemItemInfo other)
    {
        if (other == null)
            return false;
    ...
    public static bool operator ==(AD_FileSystemItemInfo x, AD_FileSystemItemInfo y)
    {
    ...
    public static implicit operator bool(AD_FileSystemItemInfo exists)
    {
        return !(exists == null);
    }
    #endregion
